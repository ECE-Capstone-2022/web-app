...
function MRecordRTC(mediaStream) {
    ...
    this.startRecording = function () {
        ...
        if (!!mediaType.audio && !!mediaType.video) {
            ...
            if (
                mediaType.audio instanceof StereoAudioRecorder &&
                !!mediaType.video
            ) {
    ...
    this.save = function (args) {
        ...
        if (!!args.audio && this.audioRecorder) {
            this.audioRecorder.save(
                typeof args.audio === "string" ? args.audio : ""
            );
        }

        if (!!args.video && this.videoRecorder) {
            this.videoRecorder.save(
                typeof args.video === "string" ? args.video : ""
            );
        }
        ...
        mediaRecorder.onerror = function (error) {
            ...
            if (!config.disableLogs) {
                ...
                if (
                    ...
                    error.name.toString().toLowerCase().indexOf("security") !==
                    -1
                ) {
    ...
    function mergeLeftRightBuffers(config, callback) {
        function mergeAudioBuffers(config, cb) {
            ...
            if (numberOfAudioChannels === 2) {
                leftBuffers = mergeBuffers(
                    leftBuffers,
                    internalInterleavedLength
                );
                rightBuffers = mergeBuffers(
                    rightBuffers,
                    internalInterleavedLength
                );
...
function CanvasRecorder(htmlElement, config) {
    ...
    this.getWebPImages = function (callback) {
        ...
        whammy.frames.forEach(function (frame, idx) {
            ...
            if (!config.disableLogs) {
                console.log(
                    framesRemaining + "/" + framesLength + " frames remaining"
                );
            }
...
function WhammyRecorder(mediaStream, config) {
    ...
    function drawFrames(frameInterval) {
        frameInterval =
            typeof frameInterval !== "undefined" ? frameInterval : 10;
    ...
    function dropBlackFrames(
        ...
        asyncLoop({
                ...
                if (!doNotCheckNext) {
                    ...
                    image.onload = function () {
                        ...
                        for (var pix = 0; pix < endPixCheck; pix += 4) {
                            ...
                            if (
                                colorDifference <=
                                maxColorDifference * pixTolerance
                            ) {
                                matchPixCount++;
                            }
                        }
            ...
            callback: function () {
                resultFrames = resultFrames.concat(
                    _frames.slice(endCheckFrame)
                );
    ...
    function whammyInWebWorker(frames) {
        ...
        function generateEBML(json) {
            ...
            for (var i = 0; i < json.length; i++) {
                ...
                var zeroes = Math.ceil(
                    Math.ceil(Math.log(len) / Math.log(2)) / 8
                );
                var sizeToString = len.toString(2);
                var padded =
                    new Array(zeroes * 7 + 7 + 1 - sizeToString.length).join(
                        "0"
                    ) + sizeToString;
            ...
            return parseInt(
                string
                    ...
                    .map(function (i) {
                        ...
                        return (
                            new Array(8 - unpadded.length + 1).join("0") +
                            unpadded
                        );
                    })
                    .join(""),
    ...
    init: function () {
        ...
        function putInDB() {
            ...
            function getFromStore(portionName) {
                transaction
                    .objectStore(self.dataStoreName)
                    .get(portionName).onsuccess = function (event) {
                    if (self.callback) {
...
function MultiStreamRecorder(arrayOfMediaStreams, options) {
    ...
    this.record = function () {
        ...
        if (
            options.previewStream &&
            typeof options.previewStream === "function"
        ) {
            options.previewStream(mixer.getMixedStream());
        }

        // record using MediaRecorder API
        mediaRecorder = new MediaStreamRecorder(
            ...
            start: function (controller) {
                ...
                video.onplaying = function () {
                    ...
                    var cameraTimer = setInterval(function f() {
                        ...
                        if (
                            controller._controlledReadableStream.state !==
                            "closed"
                        ) {
                            try {
                                controller.enqueue(
